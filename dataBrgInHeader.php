...
INNER JOIN `m_supplier` `sp` ON `brg`.`TrxInSuppIdf` = `sp`.`SupplierPK` ORDER BY `brg`.`TrxInPK` Desc");
...
<div class="container mt-4">
  ...
  <h2 class="text-center">Data Transaksi Barang Masuk</h2>
  
  <a href="formTrxInbrgHeader.php" class="btn btn-info bg-gradient mt-3">Tambah data Transaksi</a>
  ...
  <table border="1" cellpadding="10" cellspacing="0" class="table table-striped mt-3">
    ...
    <tr>
      ...
      <td><?= $row["notes"]; ?></td>
      <td>
        <a href="dataBrgInDetail.php?docNo=<?= $row["noDoc"]; ?>" class="btn btn-success bg-gradient">
          DETAIL TRANSAKSI
        </a>
      </td>
    </tr>
